...
and DeviationGraph(score_info: ScoreInfo, stats: ScoreScreenStats ref) =
    ...
    member private this.DrawHits() =
        ...
        Text.draw_aligned_b(Style.font, sprintf "Tallest = %u" biggest_bar, 25.0f, this.Bounds.CenterX, this.Bounds.Top - 45f, Colors.text, Alignment.CENTER)
        ...
        for (ms, count) in bars do
            ...
            let barHeight = 
                if DeviationGraphSettings.sqrt_scale.Value then -sqrt(float32 count  / float32 biggest_bar) * this.Bounds.Height 
                    else -(float32 count  / float32 biggest_bar) * this.Bounds.Height
